...
export default {
    ...
    watch: {
        selectedKategoriKegiatan: function (value) {
            axios.get("/api/kegiatan/" + this.selectedKategoriKegiatan).then((response) => {
                this.jenis_kegiatan = response.data.data;
            });
            this.ref_point = 0;
        },
        selectedJenisKegiatan: function (value) {
            axios.get("/api/jeniskegiatan/" + this.selectedJenisKegiatan).then((response) => {
    ...
    mounted() {
        ...
        if (this.$route.params.id) {
            this.axios.get("/api/portofolio/" + this.$route.params.id).then((response) => {
